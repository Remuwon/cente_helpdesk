(() => {
  // ../cente_helpdesk/cente_helpdesk/public/js/centenary_boot.js
  (function() {
    "use strict";
    ...
    const SKIP_TAGS = /* @__PURE__ */ new Set(["SCRIPT", "STYLE", "TEXTAREA", "INPUT", "CODE", "PRE"]);
    const STATUS_LABELS = /* @__PURE__ */ new Set(["open", "resolved", "replied", "closed", "paused", "fulfilled"]);
    ...
    function annotateStatusPills() {
      ...
      document.querySelectorAll("#app #list-rows div.flex.items-center.justify-start.w-full").forEach((item) => {
        const label = normalizeText(item.textContent);
        if (STATUS_LABELS.has(label)) {
          item.setAttribute("data-cente-status", label);
        }
    ...
    function annotateAuxiliaryPanels() {
      document.querySelectorAll("#app button, #app a").forEach((item) => {
        ...
        const panel = item.closest(".rounded") || item.closest("[class*='shadow']") || ((_a = item.parentElement) == null ? void 0 : _a.parentElement) || item.parentElement;
        if (panel) {
    ...
    function patchMeta() {
      document.querySelectorAll('meta[name="application-name"], meta[property="og:site_name"]').forEach((element) => {
...
//# sourceMappingURL=cente_helpdesk.bundle.YFVDJZMP.js.map
